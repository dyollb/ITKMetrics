...
#include "itkProductSumImageToImageMetricv4.h"
...
CreateTestImage(typename TImageType::SizeType size, double baseValue = 100.0, double multiplier = 1.0)
{
  ...
  while (!iterator.IsAtEnd())
  {
    ...
    double    value = baseValue + multiplier * (index[0] + index[1]);
...
itkProductSumImageToImageMetricv4Test(int argc, char * argv[])
{
  ...
  using MetricType = itk::ProductSumImageToImageMetricv4<ImageType, ImageType, ImageType>;
  auto metric = MetricType::New();

  // Test basic object methods
  ITK_EXERCISE_BASIC_OBJECT_METHODS(metric, ProductSumImageToImageMetricv4, ImageToImageMetricv4);
  ...
  try
  {
    ...
    // Expected value should be 5 * 3 * number_of_pixels = 15 * 100 = 1500
